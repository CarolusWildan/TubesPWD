...
class FineController
{
    ...
    // route: ?controller=fine&action=index
    // ==================================
    ...
    // route: ?controller=fine&action=show&id=1
    // ==================================
    ...
    // route: ?controller=fine&action=byReturn&return_id=3
    // ==================================
    public function byReturn(int $return_id): void
    {
        ...
        if (!$fine) {
            ...
            echo json_encode([
                'status'  => 'error',
                'message' => 'Fine for this return_id not found',
            ]);
            return;
        }
    ...
    // route: ?controller=fine&action=create
    // body: return_id, late_days, total_amount
    // ==================================
    public function create(): void
    {
        ...
        if ($return_id === null || $late_days === null || $total_amount === null) {
            ...
            echo json_encode([
                'status'  => 'error',
                'message' => 'Fields return_id, late_days, and total_amount are required',
            ]);
            return;
        }

        $success = $this->fineModel->create(
            (int) $return_id,
            (int) $late_days,
            (int) $total_amount
        );
        ...
        echo json_encode([
            ...
            'message' => $success ? 'Fine created' : 'Failed to create fine',
        ]);
    }

    // ==================================
    // POST: update denda
    // route: ?controller=fine&action=update&id=1
    // body: late_days, total_amount
    // ==================================
    ...
    // ==================================
    // DELETE: hapus denda
    // route: ?controller=fine&action=delete&id=1
    // ==================================
    ...
    // route: ?controller=fine&action=auto
    // body: return_id, due_date, return_date
    // ==================================
    public function auto(): void
    {
        ...
        if (!$return_id || !$due_date || !$return_date) {
            ...
            echo json_encode([
                'status'  => 'error',
                'message' => 'Fields return_id, due_date, and return_date are required',
            ]);
            return;
        }

        $success = $this->fineModel->autoFine(
            (int) $return_id,
            $due_date,
            $return_date
        );
        ...
        if (!$success) {
            echo json_encode([
                'status'  => 'error',
                'message' => 'No fine created (not late or failed to insert)',
            ]);
            return;
        }
        ...
        echo json_encode([
            'status'  => 'success',
            'message' => 'Fine created automatically based on late days',
        ]);
    }
}
